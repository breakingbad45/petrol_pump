...
const fetchAc = async () => {
  try {
    ...
    const updatedAcarray = response?.data
      ?.filter((item) => item.parenttype === "BANK")  // Filter accounts where type is 'BANK'
      .map((item) => ({
        value: item.id,
        label: item.name,
...
<table width="100%">
  <tbody><tr align="center"> 
      ...
      <td rowSpan={2}><em><strong><img src="assets/transaction.svg" alt style={{height: 150, padding: 6}} onerror="this.onerror=null;this.src='../assets/img/profile.png';" /></strong></em></td>
    </tr>
 ...
 <h3  className="header-table" style={{textAlign: 'center', marginTop: 15, marginBottom: 10}}> ব্যাংক স্টেটমেন্ট  </h3>
    ...
    <table className="statement-table">
        <thead>
            <tr style={{ backgroundColor: '#e0e7f5', color: '#333' }}>
                ...
                <th style={{ fontWeight: 'bold', textAlign: 'center' }}>জমা</th>
                <th style={{ fontWeight: 'bold', textAlign: 'center' }}>উত্তোলন</th>
        ...
        <tbody>
            <tr style={{ color: 'red' }}>
                <td style={{ fontWeight: 'bold', textAlign: 'center' }} colSpan="4">পূর্বের ব্যালান্স  </td>
                ...
                <tr key={i}>
                    ...
                    <td style={{ textAlign: 'left' }}>
                        {ledger.payment === '0' && ledger.sell_type === 'TRANSACTION' ? (
                            <span><strong>উত্তোলন</strong>---{ledger.remarks}</span>
                        ) : ledger.receive === '0' && ledger.sell_type === 'TRANSACTION' ? (
                            <span><strong>জমা</strong>---{ledger.remarks}</span>
                        ) : (
                            ledger.remarks
                        )}
                    </td>
