...
export default function CreditReport() {
  ...
  const handleShowDates = async () => {
    ...
    try {
      const response = await axiosInstance.get(`commondata/fetchCredit.php?type=${salesType}&balance=${balanceType}`);
  ...
  const totalReceive = filterMenu.reduce((total, item) => total + parseInt(item.balance), 0);
  ...
  const handleSubtypeFilter = (subtype) => {
    console.log(saleReport[0].balance>46110);
    // Filter filterMenu based on subtype
    let filteredData;
    if (subtype === 'PAY') {
      filteredData = saleReport.filter(item =>item.balance< 0);
    } else if (subtype === 'REC') {
      filteredData = saleReport.filter(item =>item.balance > 0);
    } else {
      filteredData = saleReport.filter(item =>item.balance!== 0);
  ...
  <div className="panel panel-default">
   ...
   <button type="button" className="btn btn-default"  onClick={() => handleSubtypeFilter('ALL')}>All</button>
   <button type="button" className="btn btn-default"   onClick={() => handleSubtypeFilter('PAY')}>Payable</button>
   <button type="button" className="btn btn-default" onClick={() => handleSubtypeFilter('REC')}>Receiveable</button>
      ...
      <table className="table" >
            <thead style={{background:'#FFF8E1'}}>
              <tr>
                ...
                <th style={{ border: '1px solid #79a4ca' }} scope="col">একাউন্টের নাম  </th>
                <th style={{ border: '1px solid #79a4ca' }} scope="col">ধরন  </th>
                <th style={{ border: '1px solid #79a4ca' }} scope="col">ঠিকানা</th>
                <th style={{ border: '1px solid #79a4ca' }} scope="col"> মোবাইল  </th>
                <th style={{ border: '1px solid #79a4ca' }} scope="col"> ব্যালান্স   </th>
            ...
            <tbody>
              {filterMenu?.map((item, index) => (
                <tr key={index}>
                  ...
                  <td style={{border: '1px solid #79a4ca'}}>{item.type}</td>
                  <td style={{border: '1px solid #79a4ca'}}>{item.address}</td>
                  <td style={{border: '1px solid #79a4ca'}}>{item.contact}</td>
                  <td style={{border: '1px solid #79a4ca',textAlign:'right',color: parseFloat(item.balance) > 0 ? 'green' : 'red'}}>{item.balance==='0'?'.00':numberWithCommas(parseFloat(item.balance).toFixed(2))}</td>
