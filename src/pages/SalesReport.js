...
export default function SalesReport() {
  ...
  const handleShowDates = async () => {
    ...
    try {
      const response = await axiosInstance.get(`commondata/fetchSales.php?type=${salesType}&start_date=${start_date}&end_date=${end_date}`);
  ...
  const totalReceive = filterMenu.reduce((total, item) => total + parseInt(item.total_tk), 0);
  ...
  const handleSubtypeFilter = (subtype) => {
    ...
    const filteredData = saleReport.filter(item => item.bill_type === subtype);
  ...
  return (
    ...
    <div className="panel panel-default">
      <div className="panel-heading" style={{ padding: '10px 15px', backgroundColor: '#77c5c5', color: '#4a235a', fontSize: '18px', display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
        <strong>Sales/Purchase Report</strong>
        {/* Navigation Buttons in Heading */}
        <div className="btn-group">
        <button type="button"    onClick={() => handleSubtypeFilter('CASH')} className="btn btn-default">Cash</button>
          <button type="button"  onClick={() => handleSubtypeFilter('CREDIT')} className="btn btn-default">Credit</button>
      ...
      <div className="panel-body">
        ...
        <form className="form-inline" style={{ marginBottom: '20px' }}>
          <div className="form-group">
            <label htmlFor="account" style={{ marginRight: '10px' }}>Sales Type</label>
            <select id="account" value={salesType}  onChange={handleChange} className="form-control">
        ...
        <table className="table" >
              <thead style={{background:'#9dc3f2'}}>
                <tr>
                  ...
                  <th style={{ border: '1px solid #79a4ca' }} scope="col">পরিমাণ</th>
              ...
              <tbody>
                {filterMenu?.map((item, index) => (
                  <tr key={index}>
                    ...
                    <td style={{border: '1px solid #79a4ca',textAlign:'right'}}>{item.unit_2==='0'?'.00':item.unit_2}</td>
