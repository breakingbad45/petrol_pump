...
const getUserData = (event) => {
  ...
  setInputsForm((prev) => {
   ...
   updated.unit_2 = ((parseFloat(value) || 0) / rate).toFixed(2);
...
;
 ...
 if(bill_type==="CREDIT"){
  ...
  useEffect(() => {
    ...
    const { id,e_date, bill_type,product_name,product_id,ac_id,sell_type,unit_2,license_no,s_rate,total_tk,remarks} = cartedit || {};
...
return () => {
  ...
  // console.log(inputsForm.license_no,pStock);
    // If sell_type is "SELL", check if unit_2 exceeds available grade stock
...
useEffect(()=>{
  ...
  const handleKeyPress = (event, index,value) => {
    ...
    const isUnit1Input = currentElement.getAttribute('name') === 'unit_2';
  if(isUnit1Input){
    if (event.key === 'Enter' &&
...
//   }, [inputsForm.product_id])
  ...
  // useEffect(() => {
  //   setInputsForm(prevInputsForm => {
  //     const newUnit2 = inputsForm.unit_2 === '0' ? inputsForm.license_no : (inputsForm.unit_2 * unitcal || 0);
  //     return {
  //       ...prevInputsForm,
  //       // license_no: newUnit2,
  ...
  //     };
  //   });
  // }, [inputsForm.license_no,inputsForm.unit_2, unitcal]);
...
}, [inputsForm.unit_2, inputsForm.s_rate, inputsForm.license_no]);
